...
export function Navbar() {
  ...
  const handleLogout = () => { logout(); nav('/login'); setMobileOpen(false); setDropdownOpen(false); };

  return (
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-40">
      <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2">
          <Wrench size={26} className="text-primary-600"/>
        ...
        <div className="hidden md:flex items-center gap-6">
          {links.map(l => (
            <Link key={l.to} to={l.to}
              className={`flex items-center gap-2 text-sm font-medium transition-colors ${isActive(l.to) ? 'text-primary-600' : 'text-gray-600 hover:text-primary-600'}`}>
          ...
          <div className="flex items-center gap-2 pl-4 border-l border-gray-200">
            {user && isCustomer ? (
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl border border-gray-200 shadow-lg py-2 z-50">
                    ...
                    {PROFILE_MENU.map(item => (
                      <Link key={item.to} to={item.to} onClick={() => setDropdownOpen(false)}
                        className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-gray-50 ${isActive(item.to) ? 'text-primary-600 font-medium' : 'text-gray-700'}`}>
              ...
              <>
                <Link to="/login" className="text-sm font-medium text-gray-600 hover:text-primary-600">Masuk</Link>
        ...
        <div className="md:hidden px-4 pb-4 space-y-1 border-t border-gray-100">
          {links.map(l => (
            <Link key={l.to} to={l.to} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium ${isActive(l.to) ? 'bg-primary-50 text-primary-600' : 'text-gray-600 hover:bg-gray-50'}`}>
            ...
            <div className="border-t border-gray-100 pt-2">
              {PROFILE_MENU.map(item => (
                <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium ${isActive(item.to) ? 'bg-primary-50 text-primary-600' : 'text-gray-600 hover:bg-gray-50'}`}>
          ...
          <div className="border-t border-gray-100 pt-2">
            {user
              ...
              : <><Link to="/login" onClick={() => setMobileOpen(false)} className="block px-4 py-3 text-sm text-gray-600 hover:bg-gray-50 rounded-xl">Masuk</Link>
