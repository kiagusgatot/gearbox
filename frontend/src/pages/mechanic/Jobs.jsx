...
export function MechanicJobs() {
  ...
  const filtered = list
    ...
    .filter(b => !q || [b.service?.name, b.user?.name, b.vehicle?.plate]
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 lg:px-8">
      <div className="max-w-full mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <Input icon={Search} placeholder="Cari job..." value={q} onChange={e => setQ(e.target.value)} className="max-w-xs"/>
        </div>
        ...
        {loading ? <Loading/> : filtered.length === 0 ? <EmptyState icon={ClipboardList} title="Tidak ada job"/> : (
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 text-xs text-gray-500">
              {filtered.length} job ditampilkan
            </div>
          </div>
        )}
      </div>
