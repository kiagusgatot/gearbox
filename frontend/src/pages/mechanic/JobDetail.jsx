  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 md:py-12 px-4">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {b.status==='ready' && <div className="card"><p className="text-sm text-blue-800 mb-2 font-medium">Lakukan inspeksi kendaraan.</p><Button onClick={()=>setShowInspect(true)}><Search size={18}/>Mulai Inspeksi</Button></div>}
        ...
        {b.status==='service_started' && <div className="card"><p className="text-sm text-sky-800 mb-2 font-medium">Admin mengirimkan perintah mulai service. Konfirmasi untuk memulai.</p><Button onClick={handleConfirmStart} loading={updating}><Play size={18}/>Konfirmasi Mulai Service</Button></div>}

        {/* ===== IN_PROGRESS: Checklist + Photo Documentation ===== */}
        {b.status==='in_progress' && <div className="card">
          <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2"><CheckSquare size={20} className="text-blue-600"/>Checklist Service</h3>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5 mb-4">
            <div className="bg-blue-600 h-2.5 rounded-full transition-all" style={{width:totalItems>0?`${(completedItems/totalItems)*100}%`:'0%'}}/>
          </div>
      ...
      <Modal isOpen={showInspect} onClose={()=>setShowInspect(false)} title="Inspeksi Kendaraan">
        <form onSubmit={handleInspection} className="space-y-5">
          ...
          <div className="p-3 bg-primary-50 border border-primary-200 rounded-xl flex justify-between items-center"><div><p className="text-xs text-gray-500">Layanan</p><p className="text-sm font-bold text-gray-900">{b.service?.name}</p></div><div className="text-right"><p className="font-extrabold text-gray-900">{formatCurrency(serviceBasePrice)}</p><p className="text-xs text-gray-500">{b.service?.estimated_duration||60} mnt</p></div></div>
