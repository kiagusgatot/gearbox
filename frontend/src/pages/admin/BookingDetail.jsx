  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 md:py-12 px-4">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {b.status==='ready' && <div className="card border-blue-200 bg-blue-50"><div className="flex items-start gap-3"><Search size={20} className="text-blue-600 mt-0.5 flex-shrink-0"/><div><p className="font-bold text-blue-900 mb-1">Menunggu Inspeksi</p><p className="text-sm text-blue-800">Mekanik sedang inspeksi kendaraan.</p></div></div></div>}
        ...
        {b.status==='service_started' && <div className="card border-sky-200 bg-sky-50"><div className="flex items-start gap-3"><Play size={20} className="text-sky-600 mt-0.5 flex-shrink-0"/><div><p className="font-bold text-sky-900 mb-1">Perintah Terkirim</p><p className="text-sm text-sky-800">Menunggu mekanik konfirmasi mulai.</p></div></div></div>}
        {b.status==='in_progress' && <div className="card border-blue-200 bg-blue-50"><div className="flex items-start gap-3"><Wrench size={20} className="text-blue-600 mt-0.5 flex-shrink-0"/><div><p className="font-bold text-blue-900 mb-1">Service Dikerjakan</p><p className="text-sm text-blue-800">Mekanik sedang mengerjakan. Menunggu checklist selesai.</p></div></div></div>}
        ...
        {b.status==='waiting_payment' && <div className="card space-y-6">
          ...
          <div><p className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><FileText size={16} className="text-primary-500"/>Detail</p><div className="bg-gray-50 rounded-xl p-4 space-y-2"><div className="flex justify-between text-sm"><span className="text-gray-600">Layanan</span><span className="font-medium">{b.service?.name}</span></div>{est?.inspection?.findings && <div className="text-sm"><span className="text-gray-600">Temuan:</span><p className="mt-1 p-2 bg-white rounded-lg text-gray-800">{est.inspection.findings}</p></div>}{items.length>0&&items.map(item=><div key={item.id} className="flex justify-between text-sm"><span className="text-gray-600">{item.name} ({item.qty}x)</span><span className="font-medium">{formatCurrency(item.total_price)}</span></div>)}<div className="flex justify-between pt-2 border-t border-gray-200"><span className="font-bold text-gray-900">Total</span><span className="text-xl font-bold text-gray-900">{formatCurrency(totalCost)}</span></div></div></div>
          <div><p className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><CreditCard size={16} className="text-orange-600"/>Konfirmasi Pembayaran</p><div className="bg-orange-50 border border-orange-200 rounded-xl p-4 space-y-4"><Input label="Nominal (Rp)" type="number" value={payForm.amount} onChange={e=>setPayForm({...payForm,amount:e.target.value})} required/><div><label className="block text-sm font-medium text-gray-700 mb-2">Metode Pembayaran</label><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{PAYMENT_METHODS.map(m=><button key={m.value} type="button" onClick={()=>setPayForm({...payForm,method:m.value})} className={`p-3 rounded-xl border-2 text-sm font-medium transition-all text-center ${payForm.method===m.value?'border-primary-500 bg-primary-50 text-gray-900 font-bold':'border-gray-200 bg-white text-gray-700 hover:border-primary-500'}`}>{m.label}</button>)}</div></div><div><label className="block text-sm font-medium text-gray-700 mb-2">Catatan (opsional)</label><textarea value={payForm.notes} onChange={e=>setPayForm({...payForm,notes:e.target.value})} className="input-field min-h-[60px]" placeholder="Nomor resi, dll"/></div><Button variant="success" fullWidth onClick={handleMarkAsPaid} loading={paying}>Konfirmasi Pembayaran Diterima</Button></div></div>
        </div>}
